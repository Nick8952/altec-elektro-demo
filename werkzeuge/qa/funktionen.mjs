...
// 5) mailto-Formular mit fiktiven Daten (kein Versand)
{
  ...
  ok("Formular: sichtbare Labels für alle Felder", await page.evaluate(() => [...document.querySelectorAll('form[action^="mailto:"] input, form[action^="mailto:"] select, form[action^="mailto:"] textarea')].every((e) => document.querySelector(`label[for="${e.id}"]`)?.offsetHeight > 0)));
  await page.evaluate(() => document.querySelector('form[action^="mailto:"] button[type="submit"]').click()); await warten(300);
  ...
  await page.type('form[action^="mailto:"] input[name="E-Mail"]', "keine-adresse");
  ...
  await page.type('form[action^="mailto:"] input[name="E-Mail"]', "test@example.invalid");
